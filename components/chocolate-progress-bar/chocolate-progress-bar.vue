<template>
	<view class="progress_box">
		...
		<view class="progress_txt">
			<view class="progress_info">{{ progress_txt.toFixed(2) }}</view>
			<text>{{ progress_deta }}</text>
		</view>
	</view>
</template>
...
export default {
	...
	onReady: function() {
		this.drawProgressbg();
		this.drawCircle(this.progress_txt / 12 * 100); //参数为1-100
	},
	methods: {
		drawProgressbg: function() {
			...
			ctx.setStrokeStyle('#ABC2F4'); // 设置圆环的颜色
		...
		drawCircle: function(step) {
			...
			gradient.addColorStop('0', '#496DF5');
			gradient.addColorStop('1.0', '#496DF5');
			ctx.setLineWidth(12);
